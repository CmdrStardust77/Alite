...
public class AndroidGraphics implements Graphics {
    ...
    private final FloatBuffer rectFromLinesBuffer;
    private final FloatBuffer circleFromLinesBuffer;
	...
	public AndroidGraphics(FileIO fileIO, float scaleFactor, Rect visibleArea, TextureManager textureManager) {
        ...
        rectFromLinesBuffer = GlUtils.allocateFloatBuffer(8 * 8);
		circleBuffer = GlUtils.allocateFloatBuffer(64 * 8);
        circleFromLinesBuffer = GlUtils.allocateFloatBuffer(128 * 8);
	...
	public void drawRect(int x, int y, int width, int height, long color) {
		...
		GLES11.glLineWidth(5 * scaleFactor);
		setGlColor(color);
		rectFromLinesBuffer.clear();
		rectFromLinesBuffer.put(x);
		rectFromLinesBuffer.put(ty);
		rectFromLinesBuffer.put(x2);
		rectFromLinesBuffer.put(ty);

        rectFromLinesBuffer.put(x2);
        rectFromLinesBuffer.put(ty);
        rectFromLinesBuffer.put(x2);
		rectFromLinesBuffer.put(ty2);
        ...
        rectFromLinesBuffer.put(x);
        rectFromLinesBuffer.put(ty);
		rectFromLinesBuffer.position(0);
		GLES11.glVertexPointer(2, GLES11.GL_FLOAT, 0, rectFromLinesBuffer);
		GLES11.glDrawArrays(GLES11.GL_LINES, 0, 8);
		GLES11.glLineWidth(1);
	}
	...
	public void drawCircle(int cx, int cy, int r, long color, int segments) {
		...
		for (float i = 0; i < 360.0f; i += step) {
			...
			circleFromLinesBuffer.put(px);
			circleFromLinesBuffer.put(py);
            if (idx != 0) {
                circleFromLinesBuffer.put(px);
                circleFromLinesBuffer.put(py);
            }
        ...
        circleFromLinesBuffer.put(cx + r);
        circleFromLinesBuffer.put(cy);
		circleFromLinesBuffer.position(0);
		setGlColor(color);
		GLES11.glVertexPointer(2, GLES11.GL_FLOAT, 0, circleFromLinesBuffer);
		GLES11.glDrawArrays(GLES11.GL_LINES, 0, segments * 2);
	}
