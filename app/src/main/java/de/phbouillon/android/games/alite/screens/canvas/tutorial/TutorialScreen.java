...
public abstract class TutorialScreen extends AliteScreen {
	...
	protected void initLines(int tutorialIndex, String... texts) {
		...
		try {
			...
			for (String t: texts) {
				lines.add(new TutorialLine(afi.getFileDescriptor(audioName), t));
				index++;
			} 
		} catch (IOException e) {
	...
	protected TutorialLine addLine(int tutorialIndex, String line) {
		...
		try {
			...
			AndroidFileIO afi = (AndroidFileIO) alite.getFileIO();
			String audioName = path + (index < 10 ? "0" + index : index) + ".mp3";
			TutorialLine result = new TutorialLine(afi.getFileDescriptor(audioName), line);
			lines.add(result);
			return result;
		} catch (IOException e) {
			AliteLog.e("Error Reading Tutorial", "Error reading Tutorial", e);
	...
	protected TutorialLine addLine(int tutorialIndex, String line, String option) {
		...
		try {
			TutorialLine result = new TutorialLine(afi.getFileDescriptor(audioName + ".mp3"), line);
			result.addSpeech(afi.getFileDescriptor(audioName + option + ".mp3"));
			lines.add(result);
			return result;
		} catch (IOException e) {
			AliteLog.e("Error Reading Tutorial", "Error reading Tutorial", e);
